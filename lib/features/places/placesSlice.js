...
export const fetchPlaceById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      ...
      console.log("I'm in fetchPlaceById")
      if (!response.ok) {
...
export const updatePlace = createAsyncThunk(
  'places/updatePlace',
  async ({ id, placeData, isApprovalAction, isEditApproval, action }, { rejectWithValue }) => {
    try {
      let url = `/api/places/${id}`;
      let requestBody = placeData;
      let method = 'PUT';
      
      // Handle approval actions
      if (isApprovalAction) {
        url = '/api/places/approve';
        method = 'POST';
        requestBody = {
          placeId: id,
          action: placeData.status === 'approved' ? 'approve' : 'reject',
          rejectionReason: placeData.rejectionReason || ''
        };
      }
      
      // Handle edit approval actions
      if (isEditApproval) {
        url = '/api/places/edit-approve';
        method = 'POST';
        requestBody = {
          placeId: id,
